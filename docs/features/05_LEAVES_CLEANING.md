...
当前推进区域生成正式叶子，下一域只显示不可处理预览。大厅独立维护600个共享钱币池位并每0.1秒激活一个空位。玩家清完所有已解锁区域后可触发`Workspace.Function.Reset.ProximityPrompt`，为这些区域创建一轮不会回锁门或改写原进度的会话钱币；任何已解锁区域未清完或上一轮未收完时服务端用英文提示拒绝。玩家装备清洁工具并持续输入后，服务端按工具行为收取或推动合法目标。

## 服务端权威

`LeafService`拥有正式区域、`ResetAreas`与Lobby的逻辑描述、空间索引、生成缺口、收取去重、区域完成、庭院广播及全服Lobby广播；`AreaCoinResetService`只绑定现有Prompt并调用权威重置入口。大厅池和重置钱币都不写`YardProgress`或DataStore，客户端模型是否加载、镜头是否可见都不能作为收取依据。
...
区域/庭院状态、会话ResetAreas与全服Lobby池 -> 服务端生成逻辑描述 -> `YardLeafSnapshot/YardLeafDelta` -> 客户端流式模型；输入与瞄准 -> 活动Remote -> 服务端同时查询当前庭院、重置钱币和Lobby -> 背包、清理统计及仅限正式区域的进度更新 -> 快照/增量刷新。
...
精确结构见`Workspace区域结构`、`ReplicatedStorage资源/叶子模板`和`收割范围与描边`。正式叶子由运行时描述驱动，不要求在Studio预摆每一片；全部直属`Workspace.Function.Reset.ProximityPrompt`都是同一重置入口。
...
- ResetAreas与正式区域共用显示和工具查询但使用不同ID；重置拾取只增加背包、清理榜和自己的200片幸运箱流，不改变门、教程、首次通关、正式区域完成值或Rebirth资格。
...
- [ ] 所有已解锁区域清完后Prompt恢复各区完整一轮钱币且门保持开启；任一区域或上一轮未清完时拒绝并显示对应英文提示。
